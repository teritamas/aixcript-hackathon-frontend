    ...
    <div class="card card-one">
      ...
      <div class="form mb-10">
        ...
        <div class="form-item">
          ...
          <p v-if="newDataset.isRecruitingTeammates">募集する</p>
          <p v-if="!newDataset.isRecruitingTeammates">募集しない</p>
        </div>
        <div class="form-item">
          <p class="form-item-label is-msg">
            <span class="form-item-label-option">任意</span>その他（500字以内）
          </p>
          <p>{{ newDataset.otherContents }}</p>
        </div>
        <button class="form-btn" @click="registerDataset()">
          上記の内容で投稿する
        </button>
        <button class="form-return-btn mt-3 mb-10" @click="returnDatasetView()">
          入力画面に戻る
        </button>
      </div>
    </div>
...
export default {
  ...
  computed: {
    ...
    registeredDatasetId() {
      return this.$store.getters["datasetStore/registeredDatasetId"];
    },
  },
  methods: {
    returnDatasetView: function () {
    ...
    registerDataset() {
      ...
      return this.$store
        .dispatch("datasetStore/registerDataset", { newDataset, file })
        .then(() => {
          ...
          setTimeout(() => {
            ...
            this.$store.commit("datasetStore/initialize");
          }, 5000);
        });
    },
    setLoading(bool) {
